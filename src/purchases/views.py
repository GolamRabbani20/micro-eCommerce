...
def purchase_start_view(request):
    ...
    if stripe_price_id is None:
        return HttpResponseBadRequest()
    purchase = purchasesProduct.objects.create(user=request.user, product=obj)
...
def purchase_success_view(request):
    ...
    if purchase_id:
        purchase = purchasesProduct.objects.get(id=purchase_id)
...
def purchase_stopped_view(request):
    ...
    if purchase_id:
        ...
        del request.session['purchase_id']
        return HttpResponseRedirect(purchase.product.get_absolute_url())
    return HttpResponse(f'stopped {purchase_id}')